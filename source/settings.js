...
var CORRECT_SIGNS = 2; // Whether we make sure to convert unsigned values to signed values.
...
var ASSERTIONS = 0; // Whether we should add runtime assertions, for example to
...
var TOTAL_STACK = 3*1024*1024; // The total stack size. There is no way to enlarge the stack, so this
                               // value must be large enough for the program's requirements. If
                               // assertions are on, we will assert on not exceeding this, otherwise,
                               // it will fail silently.
var TOTAL_MEMORY = 5*1024*1024; // The total amount of memory to use. Using more memory than this will
                                 // cause us to expand the heap, which can be costly with typed arrays:
                                 // we need to copy the old heap into a new one in that case.
var FAST_MEMORY = 0*1024*1024; // The amount of memory to initialize to 0. This ensures it will be
...
var RELOOP = 1; // Recreate js native loops from llvm data
...
var DOUBLE_MODE = 0; // How to load and store 64-bit doubles. Without typed arrays or in typed array mode 1,
...
var CORRECT_OVERFLOWS = 0; // Experimental code that tries to prevent unexpected JS overflows in integer
...
var CORRECT_ROUNDINGS = 0; // C rounds to 0 (-5.5 to -5, +5.5 to 5), while JS has no direct way to do that:
